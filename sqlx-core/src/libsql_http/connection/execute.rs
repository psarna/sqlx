...
use crate::libsql_http::connection::{ConnectionHandle, ConnectionState};
use crate::libsql_http::statement::{StatementHandle, VirtualStatement};
use crate::libsql_http::{LibsqlHttpArguments, LibsqlHttpQueryResult, LibsqlHttpRow};
use either::Either;
...
pub struct ExecuteIter<'a> {
    ...
    args: Option<LibsqlHttpArguments<'a>>,
...
fn bind(
    ...
    arguments: &Option<LibsqlHttpArguments<'_>>,
    offset: usize,
) -> Result<usize, Error> {
    let mut n = 0;
...
impl Iterator for ExecuteIter<'_> {
    type Item = Result<Either<LibsqlHttpQueryResult, LibsqlHttpRow>, Error>;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match statement.handle.step() {
            Ok(true) => {
                ...
                Some(Ok(Either::Right(LibsqlHttpRow::current(
                    &statement.handle,
                    &statement.columns,
            ...
            Ok(false) => {
                ...
                let done = LibsqlHttpQueryResult {
                    changes,
                    last_insert_rowid,
                };
