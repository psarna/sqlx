...
use crate::sqlite::{statement::unlock_notify, LibsqlHttpError};
        ...
        unsafe {
            loop {
                let status = sqlite3_exec(
                    ...
                    // callback data
                    ptr::null_mut(),
                    // out-pointer for the error message, we just use `LibsqlHttpError::new()`
                    ptr::null_mut(),
                );
                ...
                match status {
                    ...
                    _ => return Err(LibsqlHttpError::new(self.as_ptr()).into()),
                }
            }
        }
    }
    ...
    fn drop(&mut self) {
        unsafe {
            ...
            if status != SQLITE_OK {
                ...
                // SQLite handles open
                panic!("{}", LibsqlHttpError::new(self.0.as_ptr()));
            }
        }
    }
}
